...
var codeSignals = []CodeSignal{
	...
	},
	{
		...
		Handle: func(sender *Sender) interface{} {
			return "1.手机安装Alook浏览器，以下操作都在该app内完成\n2.浏览器访问 m.jd.com 登录账号\n" +
				"3.浏览器菜单左滑到第二页的工具箱-开发者工具-cookies\n4.点拷贝，发拷贝内容给QQ机器人\n" +
				"如需登录多个账号，浏览器设置-清除数据-清除Cookie后重复以上操作\n" +
				"安卓下载链接：https://down8.qwp365.cn/app/alookliulanqi.apk\n" +
				"苹果下载链接：https://apps.apple.com/cn/app/id1261944766"
		},
	},
	{
	...
	},
	{
		...
		Handle: func(sender *Sender) interface{} {
			...
			sender.handleJdCookies(func(ck *JdCookie) {
				...
				go func() {
					msgChan <- []string{ck.PtPin, ck.Query()}
				}()
			})
			if pins != nil {
				...
				for i := 0; i < len(pins); i++ {
					msgData, ok := <-msgChan
					if ok {
						msgMap[msgData[0]] = msgData[1]
					} else if time.Now().Unix()-curTime > 60 {
						break
					}
					time.Sleep(time.Millisecond * 100)
				}
				for _, pin := range pins {
					if msg, ok := msgMap[pin]; ok {
						sender.Reply(msg)
