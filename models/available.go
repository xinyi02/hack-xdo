	...
	case "1001": //ck.BeanNum
		if ui.Msg == "not login" {
			if ck.Available == True {
				...
				JdCookie{}.Push(fmt.Sprintf("失效账号\n账号：%s\n备注：%s", ck.PtPin, ck.Note))
			}
			return false
		}
	case "0":
...
func (ck JdCookie)updateCookie() string  {
	var msg string
	if ck.WsKey == "" {
		msg = fmt.Sprintf("更新失败，账号：%s，未提交wskey", ck.PtPin)
	} else {
		res := simpleCmd(fmt.Sprintf(`wskey="pin=%s;wskey=%s;" python wspt.py`, ck.PtPin, ck.WsKey))
		ss := regexp.MustCompile(`pt_key=([^;=\s]+);.*?pt_pin=([^;=\s]+);`).FindStringSubmatch(res)
		if ss != nil {
			tmpCk := JdCookie{PtKey: ss[1], PtPin: ck.PtPin}
			if  CookieOK(&tmpCk) {
				newCk, _ := GetJdCookie(ck.PtPin)
				newCk.InPool(tmpCk.PtKey)
				msg = fmt.Sprintf("更新账号：%s\n上次时间：%s\n当前时间：%s\n%s", ck.PtPin, ck.CreateAt, newCk.CreateAt, tmpCk.PtKey)
			} else {
				msg = fmt.Sprintf("更新失败，账号：%s，获取到的ck无效", ck.PtPin)
			}
		} else {
			msg = fmt.Sprintf("更新失败，账号：%s，未获取到pt_key，执行结果为：%s", ck.PtPin, res)
		}
	}
	return msg
}
